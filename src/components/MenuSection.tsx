...
function ProductCard({ product, i, favIds, onFavChange }: { product: Product; i: number; favIds: string[]; onFavChange: (id: string, isFav: boolean) => void }) {
  ...
  const fav = favIds.includes(product.id);
  ...
  const handleFav = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!user) return;
    const isNowFav = await toggleFavorite(user.id, product.id);
    onFavChange(product.id, isNowFav);
  };
  ...
  const handleQuickAdd = (e: React.MouseEvent) => {
    e.stopPropagation();
    addItemCustom(
      { id: product.id, name: product.name, description: product.description, ingredients: product.ingredients, price: product.price, image: product.image, emoji: product.emoji, badge: product.badge, tag: product.tag, extras: product.extras },
      [], "", 0, qty
    );
  ...
  return (
    <motion.div
      ...
      onClick={() => openDrawer({ id: product.id, name: product.name, description: product.description, ingredients: product.ingredients, price: product.price, image: product.image, emoji: product.emoji, badge: product.badge, tag: product.tag, extras: product.extras })}
    >
      <div className="product-card-image" style={{ position: "relative", width: "100%", aspectRatio: "1", background: "var(--bg-elevated)", overflow: "hidden" }}>
        <Image src={product.image} alt={product.name} fill sizes="(max-width: 480px) 50vw, 250px" style={{ objectFit: "cover" }} />
        {product.badge && <span style={{ position: "absolute", top: "8px", left: "8px", background: "#7B001C", color: "white", padding: "3px 8px", borderRadius: "50px", fontSize: "0.55rem", fontWeight: 700, zIndex: 2 }}>{product.badge}</span>}
        {product.tag && <span style={{ position: "absolute", top: "8px", right: "8px", background: "rgba(200,148,62,0.9)", color: "#060606", padding: "3px 8px", borderRadius: "50px", fontSize: "0.55rem", fontWeight: 700, zIndex: 2 }}>{product.tag}</span>}
      ...
      <div style={{ padding: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2px" }}>
          <h4 style={{ fontWeight: 700, fontSize: "0.85rem", color: "var(--text-primary)", lineHeight: 1.2 }}>{product.name}</h4>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.68rem", lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", marginBottom: "10px" }}>{product.description}</p>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontWeight: 800, fontSize: "0.95rem", color: "#C8943E" }}>{formatPrice(product.price)}</span>
  ...
  return (
    <section id={id} style={{ padding: "48px 16px", maxWidth: "800px", margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "12px" }}>
        {filtered.map((product, i) => <ProductCard key={product.id} product={product} i={i} favIds={favIds} onFavChange={handleFavChange} />)}
      </div>
    </section>
  );
}